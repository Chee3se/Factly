...
export default function LobbyInterface({ auth, lobbyHook, game }: Props) {
  ...
  const allPlayersReady = lobbyHook.currentLobby?.players?.every(
    (player) => player.pivot?.ready,
  );
  const canStartGame =
    isHost &&
    allPlayersReady &&
    (lobbyHook.currentLobby?.players?.length || 0) >= 1;
  const readyCount =
    lobbyHook.currentLobby?.players?.filter((p) => p.pivot?.ready).length || 0;
  const totalPlayers = lobbyHook.currentLobby?.players?.length || 0;

  // Only show delete option if lobby hasn't started and user is the only player
  const canDeleteLobby =
    !lobbyHook.currentLobby?.started && totalPlayers === 1 && isHost;
              ...
              <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                <div className="space-y-3">
                  <div className="flex items-center gap-3">
                    ...
                    <Badge
                      variant={
                        lobbyHook.currentLobby.started ? "default" : "secondary"
                      }
                    >
                      {lobbyHook.currentLobby.started
                        ? "In Progress"
                        : "Waiting"}
                    </Badge>
                  </div>
                ...
                <div className="flex gap-2">
                  ...
                  {canDeleteLobby ? (
                    ...
                    >
                      ...
                      {lobbyHook.currentLobby.started ? "Leave Game" : "Leave"}
                    </Button>
                  )}
                </div>
              </div>
            ...
            <Card>
              ...
              <CardContent>
                <LobbyPlayers
                  auth={auth}
                  lobbyHook={lobbyHook}
                  showReadyToggle={!lobbyHook.currentLobby.started}
                  isDarkTheme={false}
                  isCompact={false}
                />
              </CardContent>
