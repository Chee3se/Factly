...
export default function LobbyChat({
    ...
    const handleWheel = (e: React.WheelEvent) => {
        ...
        if ((e.deltaY > 0 && atBottom) || (e.deltaY < 0 && atTop) || scrollHeight > clientHeight) {
            e.preventDefault();
            e.stopPropagation();
        }
    };
    ...
    const getThemeClasses = () => {
        if (isDarkTheme) {
            return {
                ...
                scrollButton: 'bg-white/10 backdrop-blur-sm border-white/20 hover:bg-white/20 text-white',
        ...
        return {
            ...
            scrollButton: 'bg-background/80 backdrop-blur-sm border-border/50 hover:bg-background/90',
    ...
    return (
        ...
        >
                            ...
                            >
                                ...
                                <div className={`flex-1 min-w-0 max-w-[calc(100%-4rem)] ${
                                    ...
                                    <div className={`flex items-center gap-2 mb-1 ${
                                        ...
                                        <span className={`${isCompact ? 'text-xs' : 'text-sm'} font-medium truncate ${theme.text}`}>
                                            {message.user.name}
                                            {message.user_id === auth.user?.id && (
                                                <span className={`${theme.mutedText} ml-1`}>(You)</span>
                                            )}
                                        </span>
                                        <span className={`${isCompact ? 'text-xs' : 'text-xs'} ${theme.mutedText} flex-shrink-0`}>
                                            {formatTime(message.created_at)}
                                        </span>
                                    </div>

                                    <div className={`inline-block max-w-[80%] ${messagePadding} rounded-lg break-words overflow-wrap-anywhere hyphens-auto ${textSize} ${
                                        message.user_id === auth.user?.id
            ...
            <form onSubmit={handleSubmit} className={`flex gap-2 ${isCompact ? 'mt-3' : 'mt-4'} flex-shrink-0`}>
                <Input
                    type="text"
                    value={msg}
                ...
                <Button
                    ...
                    disabled={!msg.trim()}
                    size="sm"
                    className={isDarkTheme ? 'bg-blue-500/80 hover:bg-blue-500 border-blue-400/50' : ''}
                >
