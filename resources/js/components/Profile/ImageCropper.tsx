...
export const ImageCropper: React.FC<ImageCropperProps> = ({
  ...
  useEffect(() => {
    if (imageFile && open) {
      ...
      // For SVG and GIF, we'll handle them differently
      if (type === "image/svg+xml" || type === "image/gif") {
        // SVG and GIF: just upload without cropping
  ...
  useEffect(() => {
    if (imageSrc && fileType !== "image/svg+xml" && fileType !== "image/gif") {
      const img = new Image();
      img.onload = () => {
  ...
  }, [cropArea, imageSrc, imageSize, fileType]);
  ...
  const updatePreview = () => {
    ...
    ctx.drawImage(img, sourceX, sourceY, sourceSize, sourceSize, 0, 0, 80, 80);
    ...
    (e: MouseEvent) => {
      ...
      if (isDragging) {
        const newX = Math.max(
          0,
          Math.min(
            x - (dragStart.x * imageSize.width) / rect.width,
            imageSize.width - cropArea.size,
          ),
        );
        const newY = Math.max(
          0,
          Math.min(
            y - (dragStart.y * imageSize.height) / rect.height,
            imageSize.height - cropArea.size,
          ),
        );
        ...
        const distance =
          Math.max(Math.abs(x - centerX), Math.abs(y - centerY)) * 2;
        const maxSize =
          Math.min(
        ...
        const newSize = Math.max(
          50,
          Math.min(distance, maxSize, imageSize.width, imageSize.height),
        );
        const newX = Math.max(
          0,
          Math.min(centerX - newSize / 2, imageSize.width - newSize),
        );
        const newY = Math.max(
          0,
          Math.min(centerY - newSize / 2, imageSize.height - newSize),
        );
  ...
  const handleCrop = async () => {
    ...
    // For SVG and GIF, just upload the original file
  ...
  // For SVG and GIF, show a simple preview without cropping interface
  if (fileType === "image/svg+xml" || fileType === "image/gif") {
    return (
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent
          className="p-0 border-0 w-fit h-fit overflow-visible bg-transparent data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95"
        ...
        >
          ...
          <div className="rounded-xl bg-black p-8">
            ...
            <div className="flex flex-col items-center gap-6">
              <div className="max-w-md max-h-96 flex items-center justify-center">
                <img
                  src={imageSrc}
                  alt="Preview"
              ...
              <div className="text-center text-white space-y-2">
                <p className="text-sm">
                  {fileType === "image/svg+xml" ? "SVG" : "GIF"} files cannot be
                  cropped
                </p>
                <p className="text-xs text-gray-400">
                  The original file will be uploaded
                </p>
              </div>
              <Button
                onClick={handleCrop}
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className="p-0 border-0 -ml-11 w-fit h-fit overflow-visible bg-transparent data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95"
        showCloseButton={false}
      >
        <DialogTitle></DialogTitle>
