...
export function useFriends(authUserId?: number) {
            ...
            const channel = window.Echo.private(channelName)
                ...
                .listen('LobbyInvitationSent', (e: any) => {
                    ...
                    if (e.inviter && e.lobby_code && e.game_name) {
                        ...
                        toast.info(`${e.inviter.name} invited you to join their ${e.game_name} lobby!`, {
                            icon: "🎮",
                ...
                .error((error: any) => {
                    ...
                    if (error.type === 'AuthError') {
                        console.error('Authentication failed for private channel. Check your broadcasting auth setup.');
        ...
        try {
            ...
            if (requestData && requestData.friendUser) {
                setSentRequests(prev => {
                    const exists = prev.some(req => req.id === requestData.friendUser.id);
                    if (!exists) {
                        return [...prev, {
    ...
    const acceptFriendRequest = useCallback(async (friendRequestId: number, userId: number) => {
        setLoading(true);

        try {
            const res = await (window as any).axios.post('/friends/accept-request', {
    ...
    const declineFriendRequest = useCallback(async (friendRequestId: number, userId: number) => {
        setLoading(true);
    ...
    const inviteFriendToLobby = useCallback(async (friendId: number, lobbyCode: string) => {
        try {
            ...
            const errorMessage = error.response?.data?.message || "Failed to send lobby invitation";
            toast.error(errorMessage);
        }
    }, [friends]);
